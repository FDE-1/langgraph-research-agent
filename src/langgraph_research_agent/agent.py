...
class Agent:
    def __init__(
        ...
        self.tools_list: dict[str, BaseTool] = {
            f.name: f for f in funcs
        }
        ...
        for t in funcs:
            ...
            tool_param: FunctionToolParam = {
                ...
                "parameters": openai_schema.get("parameters", {"type": "object", "properties": {}, "additionalProperties": False}),
                "strict": True,
            }
        ...
        graph.add_conditional_edges(
            "reason",
            self.is_type_function_call_,
            {True: "action" , False: "embed"}      
        )
        graph.add_edge("action", "observe")
        graph.add_conditional_edges(
            "observe",
            self.is_max_turn,
            {True: "max_turn" , False: "reason"}
        )       
    ...
    def _max_turn(self, state: AgentState) -> dict[str, object]:
        return {"messages":[{"role": "assistant", "content": f"Max number of turn {state['turn']} was reached try to make the question shorter"}]}
